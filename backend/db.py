...
def ensure_runtime_schema() -> None:
    statements = [
        ...
        "DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'instructor_id') THEN ALTER TABLE courses ALTER COLUMN instructor_id DROP NOT NULL; END IF; END $$",
        "CREATE TABLE IF NOT EXISTS user_courses (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE, course_id UUID NOT NULL REFERENCES courses(id) ON DELETE CASCADE, proficiency VARCHAR(20) NOT NULL DEFAULT 'average', specific_topics TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), UNIQUE(user_id, course_id))",
        "ALTER TABLE user_courses ADD COLUMN IF NOT EXISTS supplementary_tutor_user_id UUID REFERENCES users(id) ON DELETE SET NULL",
        "ALTER TABLE user_courses ADD COLUMN IF NOT EXISTS strong_topics VARCHAR(120)[] NOT NULL DEFAULT '{}'",
        "ALTER TABLE user_courses ADD COLUMN IF NOT EXISTS need_topics VARCHAR(120)[] NOT NULL DEFAULT '{}'",
        ...
        "ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS weekly_progress_digest BOOLEAN NOT NULL DEFAULT TRUE",
        "ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS focus_mode_enabled BOOLEAN NOT NULL DEFAULT FALSE",
        ...
        "ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS availability_slots VARCHAR(40)[] NOT NULL DEFAULT '{}'",
        "ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS matching_preference VARCHAR(30) NOT NULL DEFAULT 'peers_only'",
        "ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS study_style_preference VARCHAR(20) NOT NULL DEFAULT 'both'",
        "ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS preferred_session_length_minutes INTEGER NOT NULL DEFAULT 60",
        "ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS include_graduate_tutors BOOLEAN NOT NULL DEFAULT FALSE",
        "ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS calendar_auto_meet BOOLEAN NOT NULL DEFAULT TRUE",
        "ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS calendar_sync_enabled BOOLEAN NOT NULL DEFAULT FALSE",
        "ALTER TABLE courses ADD COLUMN IF NOT EXISTS name VARCHAR(200)",
        "ALTER TABLE courses ADD COLUMN IF NOT EXISTS department VARCHAR(150)",
        "ALTER TABLE courses ADD COLUMN IF NOT EXISTS faculty VARCHAR(200)",
